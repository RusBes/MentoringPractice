    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            var strs = new[]
            {
                ...
                RandomStringGenerator.GenerateRandomString(5000),
            ...
            foreach (var str in strs)
            {
                ...
                ProcessAndDisplayElapsedTimeAndValidate(new StringProcessorByInput(str), true);
                ProcessAndDisplayElapsedTimeAndValidate(new StringProcessorByInput(str));
        ...
        private static void Validate(IDictionary<char, int> expected, IDictionary<char, int> actual)
        {
            ...
            foreach (var kvp in expected)
            {
                if (actual.ContainsKey(kvp.Key) && actual[kvp.Key] == kvp.Value
                    || !actual.ContainsKey(kvp.Key) && kvp.Value == 0)
                {
                    continue;
                }
